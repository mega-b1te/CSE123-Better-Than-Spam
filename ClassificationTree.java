    ...
    private ClassificationNode makeTree(Scanner sc) {
        if (!sc.hasNextLine() || sc == null) {
            ...
            if (firstLine.contains("Feature")) {
                ...
                //Adding 2 to the index of the ":" provides us with the starting index of the
                //actual feature and threshold (taking the part after "Feature: " 
                //and "Threshold: ")
                ClassificationNode node =  new ClassificationNode(new Split(firstLine.substring(
                    firstLine.indexOf(":") + 2), Double.parseDouble(threshold.substring(
                        threshold.indexOf(":") + 2))));


                node.left = makeTree(sc);
                node.right = makeTree(sc);
